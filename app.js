...
function setLanguage(lang) {
  ...
  if (document.getElementById("title"))    document.getElementById("title").textContent    = tr.title;
  if (document.getElementById("subtitle")) document.getElementById("subtitle").textContent = tr.subtitle;
...
function renderDashboard(fleetData, procData, budgetData, emailData) {
  ...
  // ── FIX: filtra procVessels SOLO alle navi presenti in data-fleet.json — esclude HQS e altre
  const procVessels = procData.vessels.filter(v => fleetIds.includes(v.vesselId));

  // ── Sync timestamp da procurement (aggiornato da N8N)
  ...
  // ── KPI globali — FIX: usa rfqOpen e dnPending (non v.rfq / v.dn)
      ...
      return `
        <div class="ship-card" onclick="location.href='vessel-detail.html?vessel=${v.id}'" style="cursor:pointer">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:.82rem;color:#89a8c5;margin-bottom:10px">
            <span>${tr("nextPort")} <strong style="color:#d7e8f6">${v.nextPort || "—"}</strong></span>
            <span>ETA <strong style="color:#d7e8f6">${v.eta ? formatDateShort(v.eta) : "—"}</strong></span>
            <span>${tr("openItems")} <strong style="color:#d7e8f6">${pv.rfqOpen || 0} proc.</strong></span>
            <span>${tr("budget")} <strong style="color:${bc.color}">${pct}%</strong></span>
          </div>
  ...
  if (procContainer) {
    // ── FIX: usa procVessels (già filtrato, senza HQS)
    ...
    procContainer.innerHTML = sortedProc.map(pv => {
      ...
      const rfqClosedPct = Math.round(
        pv.totalReq > 0
          ? ((pv.orders || 0) / pv.totalReq) * 100
          : 0
      );
      ...
      return `
        <div style="background:#0d1e30;border:1px solid ${isDelayed?"rgba(255,95,99,.4)":"#244765"};border-radius:14px;padding:16px;margin-bottom:12px">
          ...
          <div style="background:rgba(255,95,99,.08);border:1px solid rgba(255,95,99,.3);border-radius:8px;padding:8px 12px;margin-bottom:10px;font-size:.8rem;color:#ff5f63">
            🔴 <strong>${overdueCount} ${tr("itemConConsegnaScaduta")}</strong> · ${tr("dnNonRicevuto")}
          ...
          <div style="display:grid;gap:6px;margin-bottom:10px">
            <div style="display:flex;align-items:center;gap:8px;font-size:.8rem;color:#86a4c0">
              <span style="width:90px">${tr("rfqChiuse")}</span>
            ...
            <div style="display:flex;align-items:center;gap:8px;font-size:.8rem;color:#86a4c0">
              <span style="width:90px">${tr("reqToPo")}</span>
              <div style="flex:1;height:7px;background:#0b1623;border-radius:4px;overflow:hidden;border:1px solid #1e3a52">
            ...
            <div style="display:flex;align-items:center;gap:8px;font-size:.8rem;color:#86a4c0">
              <span style="width:90px">${tr("poRicevuti")}</span>
              <div style="flex:1;height:7px;background:#0b1623;border-radius:4px;overflow:hidden;border:1px solid #1e3a52">
          ...
          <div style="display:flex;flex-wrap:wrap;gap:6px;font-size:.76rem">
            ${(pv.rfqOverdue7  ||0) > 0 ? `<span style="background:#1a2a3d;border:1px solid #355d7b;border-radius:6px;padding:2px 8px;color:#89a8c5">⏱ <strong style="color:#ffbf3c">${pv.rfqOverdue7}</strong> ${tr("rfqAperte")} ${tr("rfqOverdue")}</span>` : ""}
            ${(pv.readyForPO   ||0) > 0 ? `<span style="background:#1a2a3d;border:1px solid #355d7b;border-radius:6px;padding:2px 8px;color:#89a8c5">📋 <strong style="color:#d7e8f6">${pv.readyForPO}</strong> ${tr("prontePO")}</span>` : ""}
            ${(pv.poReadyForDN ||0) > 0 ? `<span style="background:#1a2a3d;border:1px solid #355d7b;border-radius:6px;padding:2px 8px;color:#89a8c5">📦 <strong style="color:#ffbf3c">${pv.poReadyForDN}</strong> ${tr("poSenzaDN")}</span>` : ""}
            ${(pv.dnPending    ||0) > 0 ? `<span style="background:#1a2a3d;border:1px solid #355d7b;border-radius:6px;padding:2px 8px;color:#89a8c5">🚢 <strong style="color:#2fd06f">${pv.dnPending}</strong> ${tr("dnTransit")}</span>` : ""}
          </div>
        </div>`;
    }).join("");
  }
        ...
        return `
          <div style="margin-bottom:14px">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px">
              ...
              <span style="color:${color};font-weight:700;font-size:.9rem">${pct}% · €${residualK}k ${tr("residui")}</span>
            </div>
            ...
            <div style="display:flex;justify-content:space-between;margin-top:4px;font-size:.78rem;color:#86a4c0">
              <span>${tr("usato")}: €${usedK}k</span>
              <span>${tr("totale")}: €${totalK}k</span>
            </div>
          </div>`;
      }).join("");
  }
...
function renderEmailDigestCards(emailData) {
  ...
  setEl("email-captain-sub",      `${s.captainRequestsHigh || 0} ${tr("captainReqSub")}`);
  const delayEl = document.getElementById("email-delay-vessel");
  if (delayEl && s.supplierDelayVessel) {
    delayEl.textContent = `${s.supplierDelayVessel} · ${tr("ritardoDelivery")}`;
  }
}
...
function renderVesselPage(fleetData, procData, budgetData, vesselId) {
  ...
  colorEl("v-overdue7",   (procV.rfqOverdue7 || 0) > 0 ? "#ffbf3c" : "#2fd06f");
  colorEl("v-req-po-pct", (procV.pctReqToPO  || 0) < 50 ? "#ffbf3c" : "#2fd06f");
  colorEl("v-po-dn-pct",  (procV.pctPOToDN   || 0) < 30 ? "#ff5f63" : (procV.pctPOToDN || 0) < 60 ? "#ffbf3c" : "#2fd06f");
  colorEl("v-dn-transit", (procV.dnPending   || 0) > 20 ? "#ffbf3c" : "#d7e8f6");
  ...
  if (budgetV) {
    ...
    setEl("v-budget-residual", `€${residualK}k ${tr("residui")}`);
...
function renderItemsTable(procV, lang) {
  ...
  const filters = [
    ...
    },
    {
      ...
      fn: i => {
        ...
        return new Date(i.deliveryDate) < today
  ...
  // esponi al DOM per i click sui pill
